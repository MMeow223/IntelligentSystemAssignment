...
def run(
    ...
    if plots:
        ...
        # custom code
        # save confusion matrix array to a text file
        np.savetxt(save_dir / 'confusion_matrix.txt', confusion_matrix.matrix, fmt='%d')
        
        # use new_utils to get Accuracy, Precision, Recall, F1 score, MatthewsCorrelationCoefficient, and CohenKappa
        
        truth = TruthValues(confusion_matrix.matrix)
        a = Accuracy(truth)
        p = Percision(truth)
        r = Recall(truth)
        f1 = F1Score(truth)
        ...
        table = pd.DataFrame.transpose(df)
